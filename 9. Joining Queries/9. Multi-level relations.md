...
I've prepared most of the query in advance, so we just need to populate the relations object, which is the interesting part.
...
This leaves us with relations matching the three structure that you saw in the diagram earlier and that you can see on your screen right now.
...
That's just to save a bit of typing because the rest stays exactly the same.
...
So now that you've seen how to index documents in the context of multi-level relations, what about searching for them? The answer to that question is quite simple.
...
So I'm going to scroll down a bit because I have a couple of more queries down here.
...
And if I change the name to some bogus, we should see that no company is matched.
