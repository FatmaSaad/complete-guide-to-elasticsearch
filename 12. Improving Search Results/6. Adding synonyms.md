...
So if we replace the term awful with terrible, then what happens if we search for awful? A document containing either of the two terms will match because remember the queries go through the exact same analysis process.
...
Notice how I insert everything in all lowercase letters? There is a reason for that, which I'll get back to in just a second.
...
So what happens if we search for awesome? We still want the document to match.
...
But why is that? That's because when we send the query to Elasticsearch, it checks which field we are covering the description in this case.
