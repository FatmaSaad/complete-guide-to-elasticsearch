...
Are you ready? They are called central value numeric metric aggregations and multi value numeric metric aggregations.

That sounds pretty fancy, right? Don't worry, it's nowhere near as complex as it sounds.
...
Pretty easy, right? Let's move on to a query named Avg shot for the word error, which, as you can probably imagine, it calculates the average value for a given field.
...
But hey, why start with two aggregations when we can have three? Let's add two more aggregations of the types min and max.
...
Okay, this is getting out of hand, isn't it? Four aggregations in one query.
